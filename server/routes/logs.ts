    ...
    if (result.rows.length === 0 && total === 0) {
      ...
      for (const item of sampleData) {
        await pool.query(`
          ...
          VALUES ($1, $2, $3, $4, $5)
        `, [item.hash, item.file_type, item.size_mb, item.status, item.uploaded_by]);
      }
      ...
      return res.json({
        ...
        pagination: {
          ...
          pages: Math.ceil(parseInt(newCountResult.rows[0].count) / Number(limit))
        }
      });
    }
    ...
    if (result.rows.length === 0 && total === 0) {
      const sampleData = [
        {
          ...
          transaction_hash: '0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3',
        ...
        },
        {
          ...
          transaction_hash: '0xa3b2c1d4e5f6789012345678901234567890123456789012345678901234567890',
        ...
        },
        {
          ...
          transaction_hash: '0xb1c2d3e4f5a6789012345678901234567890123456789012345678901234567890',
        ...
        },
        {
          ...
          transaction_hash: '0xc2d3e4f5a6b789012345678901234567890123456789012345678901234567890',
        ...
        },
        {
          ...
          transaction_hash: '0xd3e4f5a6b7c789012345678901234567890123456789012345678901234567890',
      ...
      for (const item of sampleData) {
        await pool.query(`
          ...
          VALUES ($1, $2, $3, $4, $5, $6)
        `, [item.event_type, item.transaction_hash, item.block_number, item.gas_used, item.gas_fee, item.status]);
      }
...
router.get('/policies', async (req: AuthRequest, res: Response) => {
  try {
    ...
    let query = `
      ...
      FROM policies p 
      LEFT JOIN organizations o ON p.proposer_org_id = o.id
      WHERE 1=1
    `;
    const params: any[] = [];
    let paramCount = 0;
    ...
    if (result.rows.length === 0 && total === 0) {
      const sampleData = [
        {
          ...
          description: 'Proposed changes to age limit criteria for heart transplants',
        ...
        },
        {
          ...
          description: 'Updated consent procedures for pediatric organ donations',
          category: 'Legal',
          status: 'draft',
          votes_for: 0,
          votes_against: 0
      ...
      for (const item of sampleData) {
        await pool.query(`
          ...
          VALUES ($1, $2, $3, $4, $5, $6)
        `, [item.title, item.description, item.category, item.status, item.votes_for, item.votes_against]);
      }
