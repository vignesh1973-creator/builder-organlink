...
router.post("/signature", authenticateHospital, upload.single('signature'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    // Upload to IPFS
    const ipfsHash = await ipfsService.pinFile(
      req.file.buffer,
      fileName,
      {
        hospital_id,
    ...
    if (req.file.mimetype.startsWith('image/')) {
      try {
        ocrResult = await ocrService.advancedSignatureVerification(req.file.buffer, patient_name);
      } catch (ocrError) {
...
router.post("/verify-signature", authenticateHospital, async (req, res) => {
  try {
    ...
    if (record_type === 'patient') {
      blockchainTxHash = await blockchainService.verifyPatientSignature(record_id, verified);
    } else if (record_type === 'donor') {
      blockchainTxHash = await blockchainService.verifyDonorSignature(record_id, verified);
    } else {
      return res.status(400).json({
        success: false,
        error: "Invalid record type"
