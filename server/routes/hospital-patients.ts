...
router.post("/:patient_id/signature", authenticateHospital, async (req, res) => {
  try {
    const hospital_id = req.hospital?.hospital_id;
    const { patient_id } = req.params;
    const { signature_ipfs_hash, blockchain_tx_hash, signature_verified } = req.body;
    ...
    const result = await pool.query(
      `UPDATE patients 
       ...
       RETURNING *`,
      [signature_ipfs_hash, blockchain_tx_hash, signature_verified, patient_id, hospital_id]
    );
