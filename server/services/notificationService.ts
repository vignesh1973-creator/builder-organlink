...
export class NotificationService {
  ...
  static async createMatchNotification(patientHospitalId: string, donorHospitalId: string, patientId: string, organType: string): Promise<void> {
    try {
      // Get hospital names
      const hospitalQuery = `SELECT hospital_id, name FROM hospitals WHERE hospital_id IN ($1, $2)`;
      const hospitalResult = await pool.query(hospitalQuery, [patientHospitalId, donorHospitalId]);
      
      const hospitals = hospitalResult.rows.reduce((acc, row) => {
  ...
  static async createUrgentCaseNotification(hospitalId: string, patientId: string, patientName: string): Promise<void> {
    try {
      await this.createNotification({
  ...
  static async createMatchResponseNotification(hospitalId: string, requestId: string, response: "accepted" | "rejected", donorHospitalName: string): Promise<void> {
    try {
      const title = response === "accepted" ? "Match Request Accepted!" : "Match Request Declined";
  ...
  static async createSystemNotification(hospitalId: string, title: string, message: string): Promise<void> {
    try {
      await this.createNotification({
        hospital_id: hospitalId,
        type: "system",
  ...
  static async broadcastToAllHospitals(title: string, message: string, type: string = "system"): Promise<void> {
    try {
      const hospitalQuery = `SELECT hospital_id FROM hospitals WHERE status = 'active'`;
      ...
      console.log(`Broadcast notification sent to ${hospitalResult.rows.length} hospitals`);
    } catch (error) {
  ...
  static startDemoNotifications(): void {
    ...
    setInterval(async () => {
      try {
        const hospitalQuery = `SELECT hospital_id, name FROM hospitals WHERE status = 'active' LIMIT 1`;
          ...
          const messages = [
            {
              type: "system",
              title: "System Update",
              message: "The AI matching algorithm has been updated with the latest medical research data for improved compatibility scoring."
            },
            {
              type: "organ_match",
              title: "Potential Match Alert",
              message: "A new donor has been registered in the network. Running compatibility checks for your patients."
            },
            {
              type: "system",
              title: "Monthly Report Available",
              message: "Your monthly organ donation statistics report is now available in the Reports section."
            }
          ];
