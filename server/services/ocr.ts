...
export class OCRService {
  ...
  async advancedSignatureVerification(imageBuffer: Buffer, expectedPatientName?: string): Promise<{
    isValid: boolean;
    extractedText: string;
    confidence: number;
  ...
  async extractMedicalFields(imageBuffer: Buffer): Promise<{
    ...
    try {
      ...
      return {
        ...
        signature: text.toLowerCase().includes('signature') || text.toLowerCase().includes('signed'),
